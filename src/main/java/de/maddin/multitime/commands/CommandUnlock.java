...
public class CommandUnlock implements Command {
    ...
    public boolean run(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        affectedWorlds.forEach(world -> {
            if (!isTimeLockedInWorld(world)) {
                sender.sendMessage(getMessage("time_unlock_already_unlocked", world.getName()));
            } else {
                world.setGameRule(GameRule.DO_DAYLIGHT_CYCLE, true);
                sender.sendMessage(getMessage("time_unlock_success", world.getName()));
            }
        });
        return true;
    }
    ...
    public String getHelp() {
        return getMessage("time_unlock_help", COMMAND);
    }
}

...
class UnlockTest {
    ...
    void no_parameter_should_unlock_current_world() {
        ...
        verify(worldMock).setGameRule(GameRule.DO_DAYLIGHT_CYCLE, true);
        verify(worldMock).getName();
        verify(playerMock).sendMessage(format("Unlocked time in world \u00A7b%s\u00A7f.", TEST_NAME));
    ...
    void already_unlocked_world_should_return_info_message() {
        ...
        verify(worldMock).getGameRuleValue(GameRule.DO_DAYLIGHT_CYCLE);
        verify(worldMock).getName();
        verify(playerMock).sendMessage(format("Time in world \u00A7b%s\u00A7f is already unlocked.", TEST_NAME));
        verifyNoMoreInteractions(playerMock, worldMock, serverMock);
    }
    ...
    void all_parameter_should_unlock_all_worlds() {
        ...
        verify(worldMock, times(2)).setGameRule(GameRule.DO_DAYLIGHT_CYCLE, true);
        verify(worldMock, times(2)).getName();
        verify(playerMock, times(2)).sendMessage(
                format("Unlocked time in world \u00A7b%s\u00A7f.", TEST_NAME));
        verifyNoMoreInteractions(playerMock, worldMock, serverMock);
    }
}
